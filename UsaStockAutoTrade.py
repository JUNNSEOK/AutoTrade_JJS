...
try:
    ...
    while True:
        ...
        t_sell = t_now.replace(hour=15, minute=38, second=0, microsecond=0)
        ...
        if t_start < t_now < t_sell :  # AM 09:35 ~ PM 03:38 : 매수
            ...
            if t_now.minute == 30 and t_now.second <= 5: 
                get_stock_balance()
                time.sleep(5)
        if t_sell < t_now < t_exit:  # PM 03:38 ~ PM 03:50 : 일괄 매도
            if soldout == False:
                stock_dict = get_stock_balance()
                for sym, qty in stock_dict.items():
                    market1 = "NASD"
